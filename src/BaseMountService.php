...
/**
 * A file storage service that can mount a variety of remote providers as local file systems
 */
abstract class BaseMountService extends BasePlatformRestService implements FileServiceLike, ServiceOnlyResourceLike
{
    //*************************************************************************
    //* Members
    //*************************************************************************
    ...
    /**
     * @var Adapter The adapter of the mount
     */
    protected $_adapter = null;
    /**
     * @type array The provider configuration
     */
    ...
    public function __construct( $settings = array() )
    {
        ...
        if ( null === ( $this->_provider = IfSet::get( $settings, 'provider' ) ) )
        {
            throw new \InvalidArgumentException( 'No "provider" configuration found. What do you want me to do?' );
        }
    ...
    /**
     * Retrieves any private paths from this service's credentials
     *
     * @param array $settings
     *
     * @return array
     */
    protected function _initializeAdapter( $settings = array() )
    {

        $_providerId = IfSet::get( $this->_provider, 'id', MountTypes::LOCAL_FILE_SYSTEM );
        $_stream = IfSet::getBool( $this->_provider, 'stream', false );
        $_creds = IfSet::get( $this->_provider, 'credentials', array() );

        switch ( $_providerId )
        {
            case MountTypes::LOCAL_FILE_SYSTEM:
                $_adapter = $_stream ? new Local()new Local
        }
    }
    ...
    protected function _detectResourceMembers( $resourcePath = null )
    {
        ...
        empty( $this->_adapter ) && $this->_initializeAdapter( $settings );
